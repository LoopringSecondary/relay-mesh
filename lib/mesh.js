...
function SetupTestParticipant(Setup) {
    ...
    relayer.desp = "test relayer"
    var miner = factory.newResource(NS, 'Miner', 'miner1@loopring.org');
    miner.type = "MINER"
    miner.desp = "test miner"
    var marginSplitAddress = factory.newConcept(NS, 'Address');
    marginSplitAddress.hex = "0x3ACDF3e3D8eC52a768083f718e763727b0210650"
    marginSplitAddress.network = "NEO" 
    miner.marginSplitAddress = marginSplitAddress
    var maker = factory.newResource(NS, 'Maker', 'maker1@loopring.org');
    maker.type = "MARKET_MAKER"
    maker.desp = "test maker"

    var protocol = factory.newResource(NS, 'Protocol', '0x847983c3a34afa192cfee860698584c030f4c9db1');
    protocol.network = 'NEO'
    ...
    return getParticipantRegistry(NS + '.Relayer')
        ...
        .then(function() {
            return getParticipantRegistry(NS + '.Miner');
        })
        .then(function (minerRegistry) {
            return minerRegistry.addAll([miner]);
        })
        .then(function() {
            return getParticipantRegistry(NS + '.Maker');
        })
        .then(function (makerRegistry) {
            return makerRegistry.addAll([maker]);
        })
        .then(function() {
